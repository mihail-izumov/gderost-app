...
// Штриховка: заливка, которая означает отсутствие измерения. Живёт одним
// местом, потому что стоит в двух — в ряду дней месяца и в легенде под ним;
// разойдясь шагом, они перестают читаться как одно и то же состояние.
export const hatch = (color) => ({
  // Подложка светлее холста карточки, но темнее белого: без неё столбец
  // из одних полосок терял границы и на белой карточке читался обрывком.
  // Заливка отвечает за форму, полоски — за смысл «измерения нет».
