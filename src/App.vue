...
// получает, и способы получать больше; «Ранскеил» — система, где сигналы
...
const TABS = computed(() => [
  ...
  // в собственную картину.
  {
    ...
    label: 'Прогресс',
    iconKind: 'growth',
    // Крупный заголовок вернулся: «Сигналы» и «Ультра» подписаны, и раздел
    // без имени читался продолжением предыдущего экрана.
    title: 'Прогресс',
    leadingAction: 'update',
    eyebrow: (store.state.unit || store.state.company || 'Ваш бизнес').toUpperCase(),
    eyebrowName: store.state.unit || store.state.company || 'Ваш бизнес',
  ...
  // подпись «ВАШ БИЗНЕС» над ней относилась бы не к тому.
  {
    id: 'ultra',
    label: 'Ультра',
    // Крупного заголовка нет: его роль играет знак марки с плашкой «УЛЬТРА»,
    // и два имени одного раздела подряд читались двойной шапкой. В липкой
    // полосе при прокрутке имя остаётся.
    bigTitle: false,
    iconKind: 'ultra',
    // Крупный заголовок в потоке и он же в липкой полосе при прокрутке —
    // как на остальных разделах. В таб-баре подпись короче: там она ярлык
    // места, а на самой странице — имя комплектации.
...
<template>
  ...
  <AppShell
    v-else
    :tabs="TABS"
    :theme="!subView && tab === 'ultra' ? 'ultra-dark' : ''"
    :active="subView ? '' : tab"
    :sub-view="subView"
    :sub-views="SUB_VIEWS"
