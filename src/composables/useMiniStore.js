...
import { monthLabel } from '../i18n/format.js'
  ...
  return {
    ...
    exportText() {
      ...
      const lines = [
        ...
        '',
        'Здесь цифры одного бизнеса за один месяц. Все они либо введены владельцем,',
        'либо посчитаны из введённого — ни одна не проверена внешним источником.',
        'Статус каждого числа назван отдельно, и путать статусы нельзя:',
        '',
        ...
        'одно другим нельзя. Правила расчёта, по которым сделано всё остальное:',
        '',
        '- план дня = план месяца × вес дня недели ÷ сумма весов всех дней месяца;',
        '- прогноз (приземление) = заработанное + темп закрытых дней, разнесённый',
        '  по весам оставшихся. Прогноз один, сценариев нет;',
        '- «надо» на день = остаток плана, разнесённый по весам открытых дней;',
        '- день оценивается против плана, который стоял в момент его ввода, —',
        '  поэтому в таблице дней есть колонка с этой линейкой.',
        '',
        'Выводов, рекомендаций и оценок работы бизнеса в файле нет. Приложение считает',
        'и показывает; что с этим делать, решает владелец.',
        '',
        '## Месяц',
        '',
        `Компания: ${s.company || '—'}`,
      ...
      lines.push(
        ...
        `${(s.dow_coef || []).join(', ')}`,
        '',
        `Источник: ${shapeName(s.coef_src, s.shape_id, s.shape_from)} — ${shapeStatus(s.coef_src, 0, s.shape_id, s.shape_from).note}.`,
        `Статус: ${shapeStatus(s.coef_src, 0, s.shape_id, s.shape_from).label}.`,
