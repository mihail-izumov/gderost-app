...
/**
 * Шеврон Ранскеила заданной высоты.
 *
...
/**
 * Только маска шеврона, без размера.
 *
 * Нужна там, где размер знака задаётся таблицей метрик в стилях, а не
 * кеглем соседнего текста: инлайновый `height` всегда сильнее правила
 * в медиазапросе, и связка перестала бы менять размер на широком экране.
 * Ширина при этом всё равно остаётся обязательной — пустой блок во flex-колонке
 * с `align-items: center` получает ширину по содержимому, то есть ноль.
 */
export function chevronMask() {
  return maskOf('runscale_chevron.svg')
}
