...
// Ссылка молчит, только когда в месяце нет ни одного факта. Стартовая сумма
// из анкеты — тоже факт: она стоит в проценте плана и в прогнозе, и человек,
// принёсший её при подключении, имеет чем поделиться в ту же минуту.
// Пустоту ряда дней он увидит сам — отправка идёт через предпросмотр.
// Раньше порог требовал внесённый день, и свежеподключённый с настоящей
// выручкой месяца ждал вечера, чтобы кнопка вообще появилась.
// У поводов поделиться порог свой (внесённые дни): повод говорит о динамике,
// а её без дней не бывает.
const canShare = computed(() => {
  const m = store.model.value
  return !!m && (m.enteredCount >= 1 || !!m.carry)
})
