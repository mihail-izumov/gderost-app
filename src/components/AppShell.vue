...
// обновления ради этого не нужно.
...
// Потяни-обнови. Порог намеренно большой: случайное движение пальцем при
// чтении не должно перезагружать приложение. Сопротивление квадратным корнем —
// палец уезжает дальше, чем панель, и жест ощущается упругим.
...
function onTouchMove(e) {
  ...
  pull.value = Math.min(120, Math.round(Math.sqrt(dy) * 9))
  if (e.cancelable) e.preventDefault()
}

function onTouchEnd() {
  if (pullStart !== null && pullReady.value) { hardReload(); return }
  pullStart = null
  pull.value = 0
}
...
<template>
  ...
  >
    <!-- Индикатор жеста: знак системы приезжает сверху вместе с пальцем. -->
    <div
      class="pointer-events-none absolute inset-x-0 top-0 z-30 flex flex-col items-center justify-end overflow-hidden"
    ...
    <div
      ...
      :style="pull > 0 ? { transform: `translateY(${pull}px)`, transition: 'none' } : { transition: 'transform 0.2s ease-out' }"
      @scroll="onScroll"
