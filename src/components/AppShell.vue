...
<template>
  ...
  >
    ...
    <!-- Подложка под навигацией: холст плавно набирает плотность к нижнему
         краю. Капсула стоит поверх содержимого, и на длинной таблице цифры
         подъезжали ей вплотную — читались и панель, и то, что под ней, хуже
         обеих по отдельности. Градиент отделяет одно от другого, не рисуя
         границы: тень капсулы для этого слишком локальна.
         Слой пальца не ловит и целиком за навигацией по z. -->
    <div
      class="pointer-events-none absolute inset-x-0 bottom-0 z-30 h-28"
      style="background: linear-gradient(to top, var(--bg) 22%, color-mix(in srgb, var(--bg) 70%, transparent) 58%, transparent 100%)"
      aria-hidden="true"
    ></div>
