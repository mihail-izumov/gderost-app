...
const skin = computed(() => (dark.value
  ? {
    ...
    // Текст на чёрной заливке белый, оттенок делает прозрачность, а не другой
    // цвет. Синий отсюда убран нарочно: он означает действие и стоит на кнопке
    // ниже, а подпись внутри карточки ничего не открывает — синей она читалась
    // бы нажимаемой.
    ...
    >
      ...
      >
        ...
        <!-- Цифра в круге центруется утилитой `.gr-digit` из `main.css`:
             ...
             с серединой круга. Число взято замером по снимкам, а не на глаз. -->
        <span
