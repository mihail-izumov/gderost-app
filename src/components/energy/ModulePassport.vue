...
<template>
  <section v-if="mod">
    ...
    <template v-if="!locked">
      ...
      <!-- ⚠ Режим полный, и это осознанно: на разбор уезжают настоящие числа.
           Инженеру нужна выручка по дням, и это разговор с тем, кому владелец
           её доверил сам. Тумблера здесь нет — вопрос «показывать ли суммы»
           возникает при отправке чужому человеку, а не своему. -->
      <ShareMonthButton
        class="mt-2"
        tone="action"
        mode="full"
